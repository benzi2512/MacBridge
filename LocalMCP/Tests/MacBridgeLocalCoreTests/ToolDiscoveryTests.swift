...
final class ToolDiscoveryTests: XCTestCase {
    ...
    func testCompactIndexDoesNotRepeatAliasOrFullSchemas() throws {
        ...
        let result = try ToolDiscovery.catalog(["limit": 67]), index = try rows(["limit": 67])
        XCTAssertEqual(result["catalog_count"] as? Int, 67)
        XCTAssertEqual(result["canonical_count"] as? Int, 66)
        XCTAssertEqual(result["returned_count"] as? Int, 66)
        XCTAssertEqual(result["matched_count"] as? Int, 66)
    ...
    func testStarterDoesNotHideSpecialistToolsOrMisstateCatalogIdentity() throws {
        ...
        for arguments in lookups {
            ...
            XCTAssertEqual(categories.compactMap { $0["count"] as? Int }.reduce(0, +), 66)
