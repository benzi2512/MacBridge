...
(async()=>{
  try{
    ...
    const tools=await request('tools/list');assert.equal(tools.tools.length,57);
    ...
    assert.equal(view.catalog_count,67);assert.equal(view.snapshot_stale,false);
    ...
    console.log('PASS: discovery/initialize agreement, all template MIME/URI contracts over real stdio, identical HTML, 57 tools, background progress, partial/final log, preserved handle and final drain. Host rendering not tested.');
  }finally{
