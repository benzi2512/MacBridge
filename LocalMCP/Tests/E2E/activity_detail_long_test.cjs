...
(async () => {
  ...
  try {
    ...
    assert.equal(catalog.tools.length, 77, 'combined desktop candidate catalog');
    ...
    assert(caps.text.includes('77 catalog tools'));
    ...
    const result = {status: 'PASS', scope: 'isolated exact-binary MCP receipt test; NOT normal Chat rendering',
      binary_sha256: expectedHash, catalog_sha256: catalog.catalogEpoch, tool_count: 77,
