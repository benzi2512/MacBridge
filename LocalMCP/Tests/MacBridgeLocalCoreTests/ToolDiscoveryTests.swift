...
final class ToolDiscoveryTests: XCTestCase {
    ...
    func testCompactIndexDoesNotRepeatAliasOrFullSchemas() throws {
        ...
        let result = try ToolDiscovery.catalog(["limit": 70]), index = try rows(["limit": 70])
        XCTAssertEqual(result["catalog_count"] as? Int, 70)
        XCTAssertEqual(result["canonical_count"] as? Int, 69)
        XCTAssertEqual(result["returned_count"] as? Int, 69)
        XCTAssertEqual(result["matched_count"] as? Int, 69)
    ...
    func testStarterDoesNotHideSpecialistToolsOrMisstateCatalogIdentity() throws {
        ...
        for arguments in lookups {
            ...
            XCTAssertEqual(categories.compactMap { $0["count"] as? Int }.reduce(0, +), 69)
