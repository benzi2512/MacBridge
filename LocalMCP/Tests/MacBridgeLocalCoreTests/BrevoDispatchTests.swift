...
final class BrevoDispatchTests: XCTestCase {
    func testHeldBrevoTransportDoesNotStarveProtocolMetadata() throws {
        ...
        do {
            ...
            XCTAssertEqual((catalog["tools"] as? [JSONObject])?.count, 77)
            XCTAssertEqual(Set((catalog["tools"] as? [JSONObject] ?? []).compactMap { $0["name"] as? String }).count, 77)
            let capability = try structured(io.receive(id: 4))
            XCTAssertEqual(capability["catalog_count"] as? Int, 77)
